...
void RecoTemplate2d(int SampleType = 0, int irun = 1, int debug = 0) {
  Analyzer *a = new Analyzer(SampleType, irun, 30);
  TString savepath = "results/";
  TString savename = "RecoTemplate2d";
  ...
  for (Int_t entry = a->GetStartEntry(); entry < a->GetEndEntry(); ++entry) {
    ...
    vector<TLorentzVector> Jets = a->LVJets; //Inputs
    vector<bool> BTags = a->BTags;
    m->SetLep(a->LVLeptons.at(0), a->LVMET);
