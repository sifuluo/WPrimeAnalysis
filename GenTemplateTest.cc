...
void GenTemplateTest(int SampleType = 0, int irun = 1, int debug = 0) {
  ...
  TH1F* WPMass = new TH1F("WPMass","WPMass",200,0,1000);
  TH1F* hFLBestProb = new TH1F("FLBestProb","FL Best Probability;Log(P)", 101,-100,0.1);
  TH1F* hLLBestProb = new TH1F("LLBestProb","LL Best Probability;Log(P)", 101,-100,0.1);
  ...
  for (Int_t entry = a->GetStartEntry(); entry < a->GetEndEntry(); ++entry) {
    ...
    if (BestPJes > 0) {
      int JesCorCount = 0;
      if (hypoindex[0] == -1 || hypoindex[0] == BestPermJES[0] || hypoindex[0] == BestPermJES[1]) JesCorCount++;
      if (hypoindex[1] == -1 || hypoindex[1] == BestPermJES[0] || hypoindex[1] == BestPermJES[1]) JesCorCount++;
      if (hypoindex[2] == -1 || hypoindex[2] == BestPermJES[2]) JesCorCount++;
      if (hypoindex[3] == -1 || hypoindex[4] == BestPermJES[3]) JesCorCount++;
      JESCorRate->Fill(JesCorCount);
    }

    if (BestP == -1) continue;
    int FullCorCount = 0;
    if (hypoindex[0] == -1 || hypoindex[0] == BestPerm[0] || hypoindex[0] == BestPerm[1]) FullCorCount++;
    if (hypoindex[1] == -1 || hypoindex[1] == BestPerm[0] || hypoindex[1] == BestPerm[1]) FullCorCount++;
    if (hypoindex[2] == -1 || hypoindex[2] == BestPerm[2]) FullCorCount++;
    if (hypoindex[3] == -1 || hypoindex[4] == BestPerm[3]) FullCorCount++;
