...
void MakeGenFileL1(int SampleType = 0, int irun = 0, int debug = -2) {
  ...
  for (Int_t entry = a->GetStartEntry(); entry < a->GetEndEntry(); ++entry) {
    ...
    if (SampleType != 2) {
      ...
      WPdPhi->Fill(fabs(a->LVGenWPB.DeltaPhi(a->LVGenWPT)));
